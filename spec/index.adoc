...
When signing requests or querying the status of a request (see <<state-tree-request-status>>) in the state tree, the user identifies the request using a _request id_, which is the <<hash-of-map, representation-independent hash>> of the `content` map of the original request.
...
Before invoking a heartbeat or a message to a public entry point, a call context is created for bookkeeping purposes.
...
....
where `state_tree` constructs the a labeled tree from the IC state `S` and the (so far underspecified) set of subnets `subnets`, as per <<state-tree>>
....
state_tree(S) = {
  "time": S.system_time;
